...
public final class DefaultAsyncClient implements AsyncClient {
     ...
     */
    @Override
    public CompletableFuture<UsageQueryResponse> usageQuery(UsageQueryRequest request) {
        try {
            this.handler.validateRequestModel(request);
            TeaRequest teaRequest = REQUEST.copy().setStyle(RequestStyle.RESTFUL).setAction("UsageQuery").setMethod(HttpMethod.POST).setPathRegex("/anytrans/translate/doc/usage/query").setBodyType(BodyType.JSON).setBodyIsForm(true).setReqBodyType(BodyType.FORM).formModel(request);
            ClientExecutionParams params = new ClientExecutionParams().withInput(request).withRequest(teaRequest).withOutput(UsageQueryResponse.create());
            return this.handler.execute(params);
        } catch (Exception e) {
            CompletableFuture<UsageQueryResponse> future = new CompletableFuture<>();
            future.completeExceptionally(e);
            return future;
        }
    }
