...
public final class DefaultAsyncClient implements AsyncClient {
    ...
    protected DefaultAsyncClient(ClientConfiguration configuration) {
        ...
        this.product = "AiSearchEngine";
        this.version = "2026-04-17";
        this.endpointRule = "";
     ...
     */
    @Override
    public CompletableFuture<EngineSearchResponse> engineSearch(EngineSearchRequest request) {
        try {
            this.handler.validateRequestModel(request);
            TeaRequest teaRequest = REQUEST.copy().setStyle(RequestStyle.RESTFUL).setAction("EngineSearch").setMethod(HttpMethod.POST).setPathRegex("/api/v1/platform/app/search").setBodyType(BodyType.JSON).setBodyIsForm(false).setReqBodyType(BodyType.JSON).formModel(request);
            ClientExecutionParams params = new ClientExecutionParams().withInput(request).withRequest(teaRequest).withOutput(EngineSearchResponse.create());
            return this.handler.execute(params);
        } catch (Exception e) {
            CompletableFuture<EngineSearchResponse> future = new CompletableFuture<>();
     ...
     */
    @Override
    public CompletableFuture<QaChatResponse> qaChat(QaChatRequest request) {
        try {
            this.handler.validateRequestModel(request);
            TeaRequest teaRequest = REQUEST.copy().setStyle(RequestStyle.RESTFUL).setAction("QaChat").setMethod(HttpMethod.POST).setPathRegex("/api/v1/platform/app/chat").setBodyType(BodyType.JSON).setBodyIsForm(false).setReqBodyType(BodyType.JSON).formModel(request);
            ClientExecutionParams params = new ClientExecutionParams().withInput(request).withRequest(teaRequest).withOutput(QaChatResponse.create());
            return this.handler.execute(params);
        } catch (Exception e) {
            CompletableFuture<QaChatResponse> future = new CompletableFuture<>();
            future.completeExceptionally(e);
            return future;
        }
    }
    ...
    @Override
    public ResponseIterable<QaChatResponseBody> qaChatWithResponseIterable(QaChatRequest request) {
        this.handler.validateRequestModel(request);
        TeaRequest teaRequest = REQUEST.copy().setStyle(RequestStyle.SSE).setAction("QaChat").setMethod(HttpMethod.POST).setPathRegex("/api/v1/platform/app/chat").setBodyType(BodyType.JSON).setBodyIsForm(false).setReqBodyType(BodyType.JSON).formModel(request);
        QaChatResponseBodyIterator iterator = QaChatResponseBodyIterator.create();
