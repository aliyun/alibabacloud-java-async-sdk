// This file is auto-generated, don't edit it. Thanks.
package com.aliyun.sdk.service.datalake20200710.models;

import com.aliyun.core.annotation.*;
import darabonba.core.RequestModel;
import darabonba.core.TeaModel;
import com.aliyun.sdk.gateway.pop.models.*;

/**
 * {@link GetCatalogSettingsResponseBody} extends {@link TeaModel}
 *
 * <p>GetCatalogSettingsResponseBody</p>
 */
public class GetCatalogSettingsResponseBody extends TeaModel {
    @NameInMap("CatalogSettings")
    private CatalogSettings catalogSettings;

    @NameInMap("Code")
    private String code;

    @NameInMap("Message")
    private String message;

    @NameInMap("RequestId")
    private String requestId;

    @NameInMap("Success")
    private Boolean success;

    private GetCatalogSettingsResponseBody(Builder builder) {
        this.catalogSettings = builder.catalogSettings;
        this.code = builder.code;
        this.message = builder.message;
        this.requestId = builder.requestId;
        this.success = builder.success;
    }

    public static Builder builder() {
        return new Builder();
    }

    public static GetCatalogSettingsResponseBody create() {
        return builder().build();
    }

    /**
     * @return catalogSettings
     */
    public CatalogSettings getCatalogSettings() {
        return this.catalogSettings;
    }

    /**
     * @return code
     */
    public String getCode() {
        return this.code;
    }

    /**
     * @return message
     */
    public String getMessage() {
        return this.message;
    }

    /**
     * @return requestId
     */
    public String getRequestId() {
        return this.requestId;
    }

    /**
     * @return success
     */
    public Boolean getSuccess() {
        return this.success;
    }

    public static final class Builder {
        private CatalogSettings catalogSettings; 
        private String code; 
        private String message; 
        private String requestId; 
        private Boolean success; 

        /**
         * CatalogSettings.
         */
        public Builder catalogSettings(CatalogSettings catalogSettings) {
            this.catalogSettings = catalogSettings;
            return this;
        }

        /**
         * Code
         */
        public Builder code(String code) {
            this.code = code;
            return this;
        }

        /**
         * Message
         */
        public Builder message(String message) {
            this.message = message;
            return this;
        }

        /**
         * RequestId
         */
        public Builder requestId(String requestId) {
            this.requestId = requestId;
            return this;
        }

        /**
         * Success
         */
        public Builder success(Boolean success) {
            this.success = success;
            return this;
        }

        public GetCatalogSettingsResponseBody build() {
            return new GetCatalogSettingsResponseBody(this);
        } 

    } 

}
