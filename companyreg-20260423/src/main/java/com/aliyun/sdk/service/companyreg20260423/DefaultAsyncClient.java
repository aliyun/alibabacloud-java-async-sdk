...
package com.aliyun.sdk.service.companyreg20260423;

import com.aliyun.core.http.*;
import com.aliyun.sdk.service.companyreg20260423.models.*;
...
public final class DefaultAsyncClient implements AsyncClient {
    ...
    /**
     * @param request the request parameters of QuerySuccessIcpData  QuerySuccessIcpDataRequest
     * @return QuerySuccessIcpDataResponse
     */
    @Override
    public CompletableFuture<QuerySuccessIcpDataResponse> querySuccessIcpData(QuerySuccessIcpDataRequest request) {
        try {
            this.handler.validateRequestModel(request);
            TeaRequest teaRequest = REQUEST.copy().setStyle(RequestStyle.RPC).setAction("QuerySuccessIcpData").setMethod(HttpMethod.POST).setPathRegex("/").setBodyType(BodyType.JSON).setBodyIsForm(false).setReqBodyType(BodyType.JSON).formModel(request);
            ClientExecutionParams params = new ClientExecutionParams().withInput(request).withRequest(teaRequest).withOutput(QuerySuccessIcpDataResponse.create());
            return this.handler.execute(params);
        } catch (Exception e) {
            CompletableFuture<QuerySuccessIcpDataResponse> future = new CompletableFuture<>();
